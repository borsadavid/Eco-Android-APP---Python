...
class WhoAreYou(Screen):
    def check_login(self, username_input, password_input):
        ...
        if username_input in LogIn and password_input == LogIn[username_input]:    #if the login credentials exist
            ...
            wrong_credentials = Popup(title="Error", content=Label(text="Invalid e-mail or password!",color=(133/255, 160/255, 129/255, 1)), background='images/green.png', title_color=(133/255, 160/255, 129/255, 1),
                                      title_size=25,
                                      size_hint=(0.5, 0.5))
...
class Map(Screen):     #you need to install mapview    (pip install mapview)                 #also install garden (garden install mapview ) in terminal
    def __init__(self, **kwargs):
        ...
        mapview = MapView(  #this is the map and i set the coordinates of timisoara
            ...
            size_hint = (.94, .96),
            zoom=11,
            map_source=MapSource(max_zoom=18, min_zoom=10))          #here i set the zoom limits so you cannot zoom out to globe level
        ...
        ############################################################
        plus_btn = Button(pos_hint = {'x': 0.03, 'y':0.02},
                             size_hint = (.37,.12),
                             background_normal=("Butoane/buton1.png"),
                             )

        profile_btn = Button(pos_hint = {'x': 0.6, 'y':0.02},
                             size_hint = (0.37,.12),
                             background_normal = ("Butoane/buton11.png"),
                             font_size = 25,
                            )

        list_btn = Button(pos_hint = {'x':.4 , 'y':0.02},
                          size_hint = (.2,.12),
                          background_normal=("Butoane/buton2.png"),
        ...
        def profile_press(instance):
                self.parent.current = 'profile'
        profile_btn.bind(on_press=profile_press)

        def reward_press(instance):
                self.parent.current = 'rewards'
        plus_btn.bind(on_press=reward_press)
        def list_press(instance):
            self.parent.current='list'
        list_btn.bind(on_press=list_press)

        ###################################################################
        #################################################################
        #we load all saved markers
        #def refresh_function(dt):
        def load_markers():
        ...
        def on_click(instance, touch):
            if instance.collide_point(*touch.pos) and touch.is_double_tap:                     # Get the latitude and longitude of the click and only get the coordinates if it is a click, not zoom or pan
                ...
                btn = Button(text="What`s here?",pos=(x+30,y+10), color=(133/255, 160/255, 129/255, 1),bold = True, size_hint=(0.1, 0.1), size=(100,30), background_color = (185/255, 206/255, 163/255,1), font_size=9 ,background_normal=(""))
                #if we click this button we can add info on that point
                removebtn= Button(text="x",pos=(x-45,y+10) ,color= (133/255, 160/255, 129/255, 1),size_hint= (0.1, 0.1),size=(30,30),background_color = (185/255, 206/255, 163/255,1), font_size=20,background_normal=(""))
                ...
                def on_btn_click(instance):  #in this function we say what will happen when we will click the button
                    ...
                    def on_savebtn_click(instance):  #here we say what happend when we click save
                        ...
                        markerpopups[savedmarker] = Popup(title="What's going on here?",  #we set what to pop up when we click the saved marker
                                                          ...
                                                          background_color=(165 / 255, 186 / 255, 167 / 255, 1))   #popup backrgound wich is an image
...
#######################################
                        def save_markers():
                                ...
                                marker_info[savedmarker.uid] = {  #here i make a dictionary with all of the new markers info to save it for next time
                                    "user": UserName,
                                    "lat": lat,
                                    "lon": lon,
...
class Profile(Screen):
    def __init__(self, **kwargs):
        ...
        title_label = Label(haling =("left"),text='My Profile', font_size=35, size_hint=(.05, 0.1),pos_hint={'x':0.25,'y':0.8},color=(165 / 255, 186 / 255, 167 / 255, 1),bold=True)
        ...
        self.name_label = Label(haling =("left"),text=self.get_name_label_text(), font_size=23, size_hint=(1, 0.1),pos_hint={'x':0.05,'y':0.7},color=(165 / 255, 186 / 255, 167 / 255, 1))
        self.points_label = Label(haling =("left"),text=self.get_points_label_text(), font_size=23, size_hint=(1, 0.1),pos_hint={'x':0.05,'y':0.6},color=(165 / 255, 186 / 255, 167 / 255, 1))
        self.streak_label = Label(haling =("left"),text=self.get_streak_label_text(), font_size=23, size_hint=(1,0.1),pos_hint={'x':0.05,'y':0.5},color=(165 / 255, 186 / 255, 167 / 255, 1))
        self.activity_label=Label(haling =("left"),text=self.get_activity_label_text(), font_size=23, size_hint=(1,0.1),pos_hint={'x':0.05,'y':0.4},color=(165 / 255, 186 / 255, 167 / 255, 1))
        back_button = Button(text='Back', size_hint=(1, 0.4), background_color=(1, 1, 1, 0.03), color='black')

        button_layout=BoxLayout(orientation='horizontal', spacing=0, size_hint_y=None)

        def on_back_button_press(instance):
            self.parent.current = 'map'
        back_button.bind(on_press=on_back_button_press)

        sign_out_button=Button(text='Sign out', size_hint=(1, 0.4), background_color=(1, 1, 1, 0.03), color='black')

        def on_sign_out_press(instance):
            self.parent.current='who'
        sign_out_button.bind(on_press=on_sign_out_press)
        ...
        button_layout.add_widget(sign_out_button)
        self.add_widget(background)
...
class Rewards(Screen):
    def __init__(self, **kwargs):
        ...
        back_button = Button(text='Back', size_hint=(1, 0.1), background_color=(1, 1, 1, 0.1), color='black')
    ...
    def add_reward(self, name, points):
        ...
        label = Label(text=name, size_hint_x=None, width=75, color=(133/255, 160/255, 129/255, 1), bold="true")
        container.add_widget(label)

        # Create an unlock button for the reward
        button = Button(text=f"Unlock ({points} points)", size_hint_x=None, width=200,background_color= (185/255, 206/255, 163/255,1), background_normal=(""))
    ...
    def on_unlock(self, points):
        ...
        if total_user_points >= points:
            ...
            popup = Popup(title="Reward unlocked!",content=(Label(text="Your reward code is ...",color='green')),title_color=(51/255,102/255,0,1),     #we set what to pop up when we click the saved marker
                                      title_size=25,
                                      size_hint=(0.8, 0.5),
                                      background='images/green.png')
            popup.open()
            ...
            popup = Popup(title="Not enough points!",content=(Label(text="You don't have enough points \n    to unlock this reward.",color='green')),title_color=(51/255,102/255,0,1),     #we set what to pop up when we click the saved marker
                                      title_size=25,
                                      size_hint=(0.8, 0.5),
                                      background='images/green.png')
            popup.open()
...
class List(Map,Screen):
    def __init__(self, **kwargs):
        ...
        ## back button
        back_button = Button(text='Back', size_hint=(0.15, 0.1), background_color=(1, 1, 1, 0.1), color='black')
...
class MyApp(App):
        def build(self):
            #Window.clearcolor = (165 / 255, 186 / 255, 167 / 255, 1)
